  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        <ChevronDown className={`w-3.5 h-3.5 shrink-0 transition-transform ${open ? 'rotate-180' : ''}`} style={{ color: activeColor }} />
      </button>

      {open && (
        <div className="absolute bottom-full left-0 right-0 mb-2 bg-white rounded-2xl border border-slate-100 shadow-2xl shadow-slate-200/60 overflow-hidden z-50 animate-scale-in">
            ...
            {(loaded ? providers : []).map((provider) => {
              ...
              return (
                <button
                  ...
                  style={isSelected ? { backgroundColor: `${color}10`, outline: `1px solid ${color}30` } : {}}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-slate-300 truncate font-mono">{provider.model !== 'mock' ? provider.model : 'sample data'}</p>
                  </div>
                  {isSelected && (
